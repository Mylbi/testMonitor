...
-(void)parserDidEndDocument:(NSXMLParser *)parser {
  ...
  //  NSLog(@"==%@", [[news objectAtIndex:3]objectForKey:@"description"]);// II вариант
    ...
    for (int i = 0; i<[news count]; i++) {
        ...
        NSString *sStr = [[NSString alloc] initWithString:[[news objectAtIndex:i] objectForKey: @"description"]] ;
        array4news = [sStr componentsSeparatedByString:@" "];
        
        
        for (j=0; j<[array4news count]; j++) {
            [fArr addObject:[array4news objectAtIndex:j]];
         //   NSLog(@"%@", [array4news objectAtIndex:j]);//array4n 28obj
           // NSLog(@"%d", [fArr count]); //~519
        }
    ...
    */
    ...
    for (int k=0; k<[fArr count]; k++) {
        ...
        NSString *str1 = [[NSString alloc] init];
        str1 = [fArr objectAtIndex:k];
        NSMutableArray *array4words = [[NSMutableArray alloc] init];
        [array4words addObject:str1];
        int c=0;
        
        NSMutableArray *countingArray = [[NSMutableArray alloc] init];
        
        for (int l=0; l<[array4words count]; l++) {
 ...
 ********************/
           ...
           NSLog(@"%@", [array4words objectAtIndex:l]);
            NSLog(@"%d", l);
           
            if ([array4words objectAtIndex:l]!=str1) {
                [array4words addObject:[fArr objectAtIndex:l]];
                k++;
